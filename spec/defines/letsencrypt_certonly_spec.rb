  ...
  { 'Debian' => '9.0', 'Ubuntu' => '16.04', 'RedHat' => '7.2' }.each do |osfamily, osversion|
    context "on #{osfamily} based operating systems" do
      ...
      context 'with a single domain' do
        ...
        it { is_expected.to contain_exec('letsencrypt certonly foo.example.com').with_command 'letsencrypt --text --agree-tos -n certonly -a standalone -d foo.example.com' }
      end
      ...
      context 'with multiple domains' do
        ...
        it { is_expected.to contain_exec('letsencrypt certonly foo').with_command 'letsencrypt --text --agree-tos -n certonly -a standalone -d foo.example.com -d bar.example.com' }
      end
      ...
      context 'with custom command' do
        ...
        it { is_expected.to contain_exec('letsencrypt certonly foo.example.com').with_command '/usr/lib/letsencrypt/letsencrypt-auto --text --agree-tos -n certonly -a standalone -d foo.example.com' }
      end
      ...
      context 'with webroot plugin' do
        ...
        it { is_expected.to contain_exec('letsencrypt certonly foo.example.com').with_command 'letsencrypt --text --agree-tos -n certonly -a webroot --webroot-path /var/www/foo -d foo.example.com' }
      end
      ...
      context 'with webroot plugin and multiple domains' do
        ...
        it { is_expected.to contain_exec('letsencrypt certonly foo').with_command 'letsencrypt --text --agree-tos -n certonly -a webroot --webroot-path /var/www/foo -d foo.example.com --webroot-path /var/www/bar -d bar.example.com' }
      end
      ...
      context 'with webroot plugin, one webroot, and multiple domains' do
        ...
        it { is_expected.to contain_exec('letsencrypt certonly foo').with_command 'letsencrypt --text --agree-tos -n certonly -a webroot --webroot-path /var/www/foo -d foo.example.com -d bar.example.com' }
      end
      ...
      context 'with custom plugin' do
        ...
        it { is_expected.to contain_exec('letsencrypt certonly foo.example.com').with_command 'letsencrypt --text --agree-tos -n certonly -a apache -d foo.example.com' }
      end
      ...
      context 'with custom plugin and manage cron and cron_success_command' do
        ...
        it { is_expected.to contain_cron('letsencrypt renew cron foo.example.com').with_command '/var/lib/puppet/letsencrypt/renew-foo.example.com.sh' }
        it { is_expected.to contain_file('/var/lib/puppet/letsencrypt/renew-foo.example.com.sh').with_content "#!/bin/sh\n(echo before) && letsencrypt --text --agree-tos -n certonly -a apache --keep-until-expiring -d foo.example.com && (echo success)" }
      end
      ...
      context 'without plugin' do
        ...
        it { is_expected.to contain_exec('letsencrypt certonly foo.example.com').with_command 'letsencrypt --text --agree-tos -n certonly -d foo.example.com' }
      end
      ...
      context 'when specifying additional arguments' do
        ...
        it { is_expected.to contain_exec('letsencrypt certonly foo.example.com').with_command 'letsencrypt --text --agree-tos -n certonly -a standalone -d foo.example.com --foo bar --baz quux' }
      end
      ...
      context 'with manage cron and suppress_cron_output' do
        ...
        it { is_expected.to contain_cron('letsencrypt renew cron foo.example.com').with_command '/var/lib/puppet/letsencrypt/renew-foo.example.com.sh' }
        it { is_expected.to contain_file('/var/lib/puppet/letsencrypt/renew-foo.example.com.sh').with_content "#!/bin/sh\nletsencrypt --text --agree-tos -n certonly -a standalone --keep-until-expiring -d foo.example.com > /dev/null 2>&1" }
      end
    end
  end
end
