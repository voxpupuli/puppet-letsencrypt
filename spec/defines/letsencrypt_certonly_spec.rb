...
describe 'letsencrypt::certonly' do
  on_supported_os.each do |os, facts|
    context "on #{os} based operating systems" do
      ...
      context 'with dns-gandi plugin with personnal_access_token' do
        let(:title) { 'foo.example.com' }
        let(:params) { { plugin: 'dns-gandi', letsencrypt_command: 'letsencrypt' } }
        let(:pre_condition) do
          <<-PUPPET
          ...
          class { 'letsencrypt::plugin::dns_gandi':
            package_name           => 'irrelevant',
            personnal_access_token => 'dummy-pat',
          }
          PUPPET
        end
      ...
      context 'with dns-gandi plugin without api_key or personnal_access_token' do
        let(:title) { 'foo.example.com' }
        let(:params) { { plugin: 'dns-gandi', letsencrypt_command: 'letsencrypt' } }
        let(:pre_condition) do
          <<-PUPPET
        ...
        it { is_expected.to compile.and_raise_error(%r{expects a value for parameter 'api_key' or 'personnal_access_token'}) }
      end
