  ...
  {'Debian' => '9.0', 'Ubuntu' => '16.04', 'RedHat' => '7.2'}.each do |osfamily, osversion|
    context "on #{osfamily} based operating systems" do
      ...
      context 'with custom plugin and manage cron' do
        ...
        let(:params) { { plugin: 'apache',
                         manage_cron: true } }
        it { is_expected.to contain_cron('certbot renew cron foo.example.com').with_command 'certbot --agree-tos certonly -a apache --keep-until-expiring -d foo.example.com' }
      end
      ...
      context 'with custom plugin and manage cron and cron_success_command' do
        ...
        it { is_expected.to contain_cron('certbot renew cron foo.example.com').with_command 'certbot --agree-tos certonly -a apache --keep-until-expiring -d foo.example.com && (echo success)' }
      end
